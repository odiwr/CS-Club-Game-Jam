...
# THEME VARIABLES YOU CAN CHANGE 👇
THEME_COLOR   = red      # all line art: ship, asteroids, bullets, saucer, lives
TEXT_COLOR    = white      # all on-screen text
BG_COLOR      = black      # fallback background color
BG_IMAGE_PATH = ""         # e.g. "assets/bg_space.png" ("" = no image, just BG_COLOR)
MASTER_VOLUME = 0.3        # 0.0–1.0 (was full volume before)
...
if BG_IMAGE_PATH:
    try:
        ...
        bg_image = None  # fallback to solid color if load fails


def clear_screen():
    """Draw background image if set, otherwise solid BG_COLOR."""
...
# ---------- GLOBAL VOLUME ----------
for s in (snd_fire, snd_bangL, snd_bangM, snd_bangS, snd_extra, snd_saucerB, snd_saucerS):
...
class Bullet:
    ...
    def updateBullet(self):
        ...
        # Drawing
        pygame.draw.circle(gameDisplay, THEME_COLOR, (int(self.x), int(self.y)), 3)
...
def gameLoop(startingState):
    ...
    while gameState != "Exit":
        ...
        while gameState == "Menu":
            clear_screen()
            drawText("ASTEROIDS", TEXT_COLOR, display_width / 2, display_height / 2, 100)
            drawText("Press any key to START", TEXT_COLOR, display_width / 2, display_height / 2 + 100, 50)
        ...
        if gameState != "Game Over":
            ...
            drawText("Game Over", TEXT_COLOR, display_width / 2, display_height / 2, 100)
            drawText("Press \"R\" to restart!", TEXT_COLOR, display_width / 2, display_height / 2 + 100, 50)
            live = -1

        # Draw score
        drawText(str(score), TEXT_COLOR, 60, 20, 40, False)
